...
#                 pred = model.predict(X)[0]
#                 print("Prediction Raw Output:", pred)  # Debugging print
...
# @register_keras_serializable(package="Custom", name="MeanSquaredError")
...
#         new_row = pd.DataFrame([[datetime.now().date()] + new_numbers], columns=df.columns)
#         df = pd.concat([df, new_row])
#         df.to_csv(config['csv'], index=False)

#     with st.spinner("Retraining model..."):
#         try:
#             model = tf.keras.models.load_model(config['model'], custom_objects={"MeanSquaredError": CustomMeanSquaredError})
#             # Recompile the model after loading
